...
\node[right=0.5cm of start] (n) {$P(n)$};
 \node[draw,rectangle, below=1cm of start, text width =4.8cm] (grammar) {\centering
Stochastic Grammar \\
\begin{flushleft}
...
\footnotesize$\Kbf_{\bm{\theta}} \sim
P(\Kbf_{\bm{\theta}} \mid \bm{\Omega},S,n),\;\,\bm{\theta}\subseteq\bm{\theta}^*$
\end{flushleft}
