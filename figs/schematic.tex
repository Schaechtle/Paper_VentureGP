...
\begin{tikzpicture}[thick]
 \node (base_kernels) {\small$\{\Kbf_{\bm{\theta}^1}^1,\cdots,\Kbf_{\bm{\theta}^m}^m\}$};
 \node[left=0.2cm of base_kernels] (theta) {\small$\bm{\theta}^* \sim P(\bm{\theta}^*)$};
 \node[right=0.2cm of base_kernels] (n) {\small$ n \sim P(n)$};

 \node[below=0.60 of n,xshift=-0.7cm] (nleqm) {\small$ n \leq m$};

 \node[draw,rectangle, below=1.5cm of base_kernels] (grammar) {Stochastic Grammar};

 \node[draw,rectangle,below=1.5cm of grammar] (gpmem) {\texttt{gpmem}};

 \node[draw,rectangle,below=1.5cm of gpmem] (f) {Data Generation};
\node[left of=f,xshift=-1.5cm] (x) {$\mathbf{t}$};
\node[right of=f,xshift=1.5cm] (y) {$\mathbf{f}$};

 \node[below right of=grammar,xshift=1.2cm,yshift=-0.3cm] (k)
{\small$\Kbf_{\bm{\theta}} \sim
P(\Kbf_{\bm{\theta}}),\bm{\theta}\subseteq\bm{\theta}^*$};


\node[below right of=gpmem,xshift=0.0cm,yshift=-0.3cm] (gp) {
\small$f_{emu}$};

% 1st pass: draw arrows
  \draw[thick,->] (base_kernels) -- (grammar);
  \draw[thick,->] (n) -- (grammar);
  \draw[thick,->] (theta) -- (grammar);
