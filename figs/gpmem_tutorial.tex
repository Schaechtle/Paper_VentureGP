...
\small$\text{memo table} = (\mathbf{x}_{past},\fbf)$\vspace{1mm}

\small$P(f_{emu}(x) \mid \mathbf{x}_{past},\fbf)\sim \mathcal{N}(\mu(\mathbf{x}),\mathbf{K}_\theta\big(\mathbf{x},\mathbf{x})\big)$
 };
%  \node[draw,rectangle,color=ForestGreen,below = .1ex of gpmem,minimum width=1.5cm, minimum height=0.4cm,yshift=0.9cm,xshift=0.3cm] (mark) {};
 ...
 $\text{SE}\,= \sigma^2 \exp(-\frac{(x-x^\prime)^2}{2\ell^2})$\vspace{1mm}

 $\theta \;\;\,\,= \{\sigma,\ell \} \rightarrow$ Scope\vspace{1mm}
...
\begin{tabular}{l|l}\footnotesize
  ...
  \footnotesize $x_1$  &\footnotesize$f_1$ \\
 \footnotesize  $x_2$  & \footnotesize$f_2$ \\
