...
In continuous settings, many Bayesian optimization methods deploy GPs~\citep[e.g.][]{snoek2012practical}.
...
Our implementation has two notable features: (i) the ability to search over a broader class of contexts than typical parametric families of contexts, and (ii) the parsimony of the resulting probabilistic program.
...
\end{enumerate}
Note that when $P(\theta)$ has high entropy, the chosen action $a$ may be far from optimal, but the information gained by probing action $a$ will improve the belief $\theta$.
This amounts to ``exploration.''
When $P(\theta)$ has high entropy and is concentrated on a region of belief space that closely matches the true distributions $P_\true$, exploration will be less likely to occur, but the chosen actions $a$ will tend to receive high rewards.
...
In a probabilstic programing platform, however, the representation of heterogeneously structured or infinite-dimensional context spaces is quite natural.
Any computable model of the conditional distributions $\br{P(r|a)}_{a \in \Acal}$ can be represented as a procedure-valued random variable $(\lambda (a) \ldots)$.
Thus, for computational Thompson sampling, the most general context space $\hat\Theta$ is the space of program texts.
Any other context space $\Theta$ will be a subset of $\hat\Theta$.

\subsection{Thompson sampling in Venture}
Because Venture supports sampling and inference on procedure-valued random variables (and the generative models which produce those procedures), Venture can capture arbitrary context spaces as described above.
...
Computationally, we represent a context as $\theta = (\sigma, \ell, \abf_\past, \rbf_\past)$, where
\begin{itemize}
  \item $\sigma$ and $\ell$ are hyperparameters for a smoothing kernel $K_\prior(a,a') = \sigma^2 \exp\pn{-\frac{(a-a')^2}{2\ell^2}}$
...
\end{itemize}
To simplify the treatment, we take prior mean $\mu_\prior \equiv 0$.  The mean and covariance for $\theta$ are then gotten by the usual computational procedure:
\begin{align*}
  \mu(\abf)
...
$\Theta$ is, however, quite easily representable as a computational procedure together with parameters and past samples, as we do in the representation $\theta = (\sigma, \ell, \abf_\past, \rbf_\past)$.
...
(\textbf{TODO} we should not assume $V$ is deterministic, it would be easy enough to make it random.)
...
\begin{lstlisting}[frame=single,caption={
  ...
  The answer to our maximization problem is simply the maximum $r$; but our algorithm also learns more potentially useful information.
