...
\put(150,300){{\parbox{0.4\linewidth}{%
	...
	\widetilde{r}_{i,a}i\\ 
	\widetilde{r}_{i,a}=\hat{f} &\sim \Ncal(\hat\mubf, \hat\Kbf_{\thetabf}\mid
\sigma,\ell,\abf,\rbf)\\ 
...
\Ktheta(\abf,\abf)^{-1}\rbf \big \}\\
  ...
  &=
   ...
   &\;\;\;\;\;\;\;\; \;\;\;\;\;\;\;\;  \;\;\;\;\;\;\;\;  \;\;\;\;\;\;\;\;  \;\;\;   \rbf^\top \Ktheta\pn{\abf, \abf \mvert \sigma, \ell}^{{-1}} \rbf
  \big)\bigg \}\\
   \Ktheta(\abf,\abf)&=\text{SE}=\bigg[\sigma^2 \exp \big \{{-\frac{(a_1 -
...
\[ V \sim \mathcal{GP}\big(\mubf(a), \Kbf_{\thetabf}(a,a)\big), \]
where the mean $\mubf$ is a computable function $\Acal \to \R$ and the covariance
$\Kbf_{\thetabf}$ is a computable (symmetric, positive-semidefinite) function $\Acal \times
...
\[ \ttheta = (\thetabf, \abf, \rbf) = (\bm{\mu}, \Kbf_{\thetabf},\thetabf, \abf, \rbf), \]
where $\mubf$ is a procedure to be used as the prior mean function and
$\Kbf_{\thetabf}$ is a procedure to be used as the prior covariance function, parameterized by 
$\thetabf$.
...
representation $\ttheta = (\mubf, \Kbf_{\thetabf}, \thetabf, \abf, \rbf)$.
...
\begin{align*}
  ...
  \min\bigg\{1,\ \exp\bigg( -\frac12\bigg(
    \rbf^T \Ktheta\pn{\abf, \abf \mvert \sigma', \ell'}^{-1} \rbf \\
...
and
\[ \widetilde{r}_{i,a}=\hat{f} \sim \Ncal(\hat\mubf, \hat\Kbf_{\thetabf})) \]
and $\br{\widetilde{r}_{i,a}}_{i=1}^{N_\avg}$ are i.i.d.\ for a fixed $a$.
(In the above, $\hat\mubf$ and $\hat\Kbf_{\thetabf}$ are the mean and variance of a posterior sample at the single point $\hat\xbf = (a)$.)
...
posterior variance $\hat\Kbf_{\thetabf}$ is high, it is possible (especially when
